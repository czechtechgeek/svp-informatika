# Video: Střih jednoduchého klipu

## 🎯 Vazba na RVP ZV (Informatika)
- **Oblast:** Digitální technologie
- **Výstup:** <div class="curriculumTag" data-code="I-9-3-02" style="display: inline-flex; align-items: center; gap: 6px; cursor: pointer; padding: 4px 10px; background-color: #f0f7ff; border: 1px solid #1975FE; border-radius: 6px; font-size: 13px; margin: 2px 4px 2px 0; user-select: none;"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#1975FE" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg><span style="color: #1975FE; font-weight: 500;">I-9-3-02</span><span style="color: #374151;">Žák efektivně využívá digitální nástroje pro tvorbu obsahu</span></div>
- **Výstup:** <div class="curriculumTag" data-code="I-9-4-01" style="display: inline-flex; align-items: center; gap: 6px; cursor: pointer; padding: 4px 10px; background-color: #f0f7ff; border: 1px solid #1975FE; border-radius: 6px; font-size: 13px; margin: 2px 4px 2px 0; user-select: none;"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#1975FE" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg><span style="color: #1975FE; font-weight: 500;">I-9-4-01</span><span style="color: #374151;">Žák kriticky hodnotí digitální obsah a chování online</span></div>

## 💡 Metodický tip pro pátky
Filmová analýza: pusťte 3minutový úsek libovolného YouTube videa (bez zvuku) a nechejte žáky spočítat střihy. Poté s zvukem. Diskuse: Proč je tam každý střih? Rozvíjí mediální gramotnost.

## 🎯 Cíle hodiny

- Žák vysvětlí základní pojmy střihu: klip, sestřih, přechod, titulek
- Žák sestříhá krátký videoklip (30–60 sekund) z připravených nebo vlastních záběrů
- Žák přidá k videu jednoduchý titulek a základní přechod mezi záběry
- Žák exportuje hotové video v běžném formátu (MP4)

## 💡 Metodický postup

### 1. Základy střihu: Teorie (8 min) — tabule

Klíčové pojmy:

| Pojem | Vysvětlení |
|-------|-----------|
| **Klip** | Jeden zaznamenaný záběr |
| **Sestřih / Timeline** | Seřazení klipů za sebou |
| **Střih** | Přechod z jednoho záběru na druhý |
| **Přechod (Transition)** | Animovaný přechod (fade, wipe) |
| **Titulek** | Text přes video (název, podpis) |
| **B-roll** | Doplňkové záběry bez hlavní akce |

Učitel ukáže na projektoru jednoduchou timeline ve střihovém programu a pojmenuje části.

Klíčové pravidlo: „Dobrý střih není vidět. Špatný střih vypadá jako chyba."

### 2. Demo: Vytvoření krátkého videa (12 min) — tabule

Učitel živě sestříhá 2–3 připravené klipy (volně licencovaná videa z Pexels.com):

**Kroky v Clipchamp (Windows) nebo Canva Video:**
1. Importovat klipy
2. Přetáhnout na timeline v pořadí
3. Ořezat začátek/konec klipu (drag handles)
4. Přidat přechod mezi klipy
5. Vložit titulek na začátek
6. Přidat hudbu na pozadí (volně licencovaná)
7. Exportovat jako MP4

Žáci říkají, co přidat nebo změnit.

### 3. Střih vlastního klipu (22 min) — PC

Žáci pracují individuálně nebo ve dvojicích. Zdroje videomateriálu:

**Varianta A — připravené klipy:**
- Učitel poskytne složku se 6–8 voně licencovanými klipy (Pexels, Pixabay)
- Žáci z nich sestaví 30–60s videopříběh

**Varianta B — vlastní záznamy:**
- Žáci mohou použít klipy ze svého mobilu (nevhodný obsah = nesmiřitelné ne)
- Nebo screen recording PC aktivity

**Povinné prvky:**
- Min. 3 klipy za sebou
- Alespoň jeden ořez (zkrácení klipu)
- Titulek na začátku nebo konci
- Export jako MP4

**Doporučené nástroje:**
- **Clipchamp** (Windows 10/11 — zabudovaný, zdarma)
- **Canva Video** ([canva.com](https://www.canva.com)) — online, zdarma
- **CapCut Online** — populární u žáků, zdarma

### 4. Přehrání a zpětná vazba (3 min)

Dobrovolník přehraje hotové video. Třída identifikuje: Kde jsou střihy? Co funguje? Co by změnili?

Učitel informuje: „Video se vám bude hodit v projektu — třídní digitální časopis bude mít i videokomponenty."

## 📂 Podklady

- **Clipchamp (Windows):** Zabudovaný editor videa ve Windows 10/11 — Start → Clipchamp
- **Canva Video:** [canva.com](https://www.canva.com) → Videa — online, bez instalace
- **Volně licencovaná videa:** [pexels.com/videos](https://www.pexels.com/videos/) a [pixabay.com/videos](https://pixabay.com/videos/) — Creative Commons
- **Volně licencovaná hudba:** [freemusicarchive.org](https://freemusicarchive.org) nebo [pixabay.com/music](https://pixabay.com/music/) — pro pozadí
- **Technické pojmy:** Rozlišení videa (720p, 1080p), snímky za sekundu (fps), formáty (MP4, MOV, AVI)

!!! tip "Tip pro učitele"
    Střih videa je časově náročnější než ostatní aktivity — realisticky očekávejte nedokončené projekty. Zdůrazněte, že cílem je naučit se workflow, ne vytvořit profesionální video. Pokud žáci nemají vhodný videomaterialál, vždy mějte připravenou sadu volně licencovaných klipů na USB nebo sdíleném drivu. CapCut je populární, ale má omezení pro školní použití (vyžaduje účet) — ověřte dostupnost předem.
